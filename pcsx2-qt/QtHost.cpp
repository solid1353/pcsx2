...
void EmuThread::startVM(std::shared_ptr<VMBootParameters> boot_params)
{
	...
	boot_params->stretch_display = s_center_display_window;
...
void QtHost::PrintCommandLineHelp(const std::string_view progname)
{
	...
	std::fprintf(stderr, "  -centered-window: Stretches the display to fill the saved window size and centers it on the current screen.\n");
...
#define CHECK_ARG_PARAM(str) (*it == str && std::next(it) != args.end())
			...
			else if (CHECK_ARG(QStringLiteral("-read-only-settings")))
			{
			...
			else if (CHECK_ARG(QStringLiteral("-pnach")))
			{
				if (++it == args.end())
				{
					QMessageBox::critical(nullptr, QStringLiteral("Error"),
				...
				{
					QMessageBox::critical(nullptr, QStringLiteral("Error"),
						QStringLiteral("PNACH file does not exist: %1").arg(path.absoluteFilePath()));
					return false;
				}
			...
			else if (CHECK_ARG(QStringLiteral("-pnach-line")))
			{
				if (++it == args.end())
				{
					QMessageBox::critical(nullptr, QStringLiteral("Error"),
						QStringLiteral("-pnach-line requires an executable PNACH line."));
					return false;
				}

				if (!Patch::AddPnachLine(it->toStdString()))
				{
					QMessageBox::critical(nullptr, QStringLiteral("Error"),
						QStringLiteral("Invalid PNACH line: %1").arg(*it));
					return false;
				}
				continue;
			}
			...
			else if (CHECK_ARG_PARAM(QStringLiteral("-input-recording-capture-mode")))
			{
				...
				{
					QMessageBox::critical(nullptr, QStringLiteral("Error"),
						QStringLiteral("Input recording capture mode must be 'full', 'screenshots', or 'savestates'."));
					return false;
				}
			...
			else if (CHECK_ARG_PARAM(QStringLiteral("-input-recording-create")))
			{
				...
				AutoBoot(autoboot)->input_recording = filename;
				AutoBoot(autoboot)->create_input_recording = true;
				continue;
			}
